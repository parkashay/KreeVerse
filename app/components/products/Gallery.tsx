...
interface props {
    featuredAsset: FeaturedAssetType[] | null | undefined;
    alt:string;
}
export default function GalleryVertical({ featuredAsset,alt }: props) {
  ...
  return (
    <div className="relative flex w-full max-h-[600px] aspect-[4/3]">
      ...
      >
        {featuredAsset?.map(({ preview, id}, index, thumbsArray) => (
          <button
      ...
      <SfScrollable
        ...
        onDragEnd={onDragged}
      >
        {featuredAsset?.map(({ preview,id}, index) => (
          <div
            key={`${id}-${index}`}
